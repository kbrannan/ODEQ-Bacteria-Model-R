...
df.output <- onsite_pets(chr.wrkdir=chr.onsite.pets.dir,chr.input="OnSitePets06.txt")
## land use information
chk.lu.RAOCUT.area <- 145.2 # in acres
## general
chk.sqolim.fac      <- 9 # unitless
## onsite information
chk.onsite.bac.prod        <- 7.6000000E+11 # orgs/sys-day
chk.num.hh <- 156
chk.near.stream.strc <- 13
...
chk.onsite.to.stream <- 1.00E-04 # in percent
# pets
chk.pets.hh <- 1
...
chr.chk <- c(paste0("Standard Error for In-Stream     = ", 
             ...
             paste0("Standard Error for RAOCUT Accum = ", 
                    sum((chk.accum.RAOCUT - df.output$Accum.RAOCUT)^2)/12))
cat(chr.chk, file = paste0(chr.onsite.pets.dir, "/onsite-pets-std-error-cowcalf06.txt"), sep="\n")
