...
df.output <- onsite_pets(chr.wrkdir=chr.onsite.pets.dir,chr.input="OnSitePets14.txt")
## land use information
chk.lu.RAOCUT.area <- 213.6 # in acres
## general
...
chr.chk <- c(paste0("Standard Error for In-Stream     = ", 
             ...
             paste0("Standard Error for RAOCUT Accum = ", 
                    sum((chk.accum.RAOCUT - df.output$Accum.RAOCUT)^2)/12))
cat(chr.chk, file = paste0(chr.onsite.pets.dir, "/onsite-pets-std-error-cowcalf14.txt"), sep="\n")
