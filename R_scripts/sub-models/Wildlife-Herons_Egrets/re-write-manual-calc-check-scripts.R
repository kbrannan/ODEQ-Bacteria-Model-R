...
chr.script.file.name <- "Wildlife_HerEgr_Sub_Model_calculation_check-subwtsd-update.R"
chr.script <- scan(file = paste0(chr.wildlife.heregr.dir, "/",
                                 chr.script.file.name),
                   what = "character", sep = "\n")

chr.files <- list.files(path = chr.wildlife.heregr.dir, pattern = "^wildlifeHerEgr([0-9]){2}\\.txt$",
                        full.names = TRUE)
...
for(ii in 1:length(chr.files)) {
  ...
  chr.input <- paste0("wildlifeHerEgr", tmp.sub ,".txt")
  ...
  cat(tmp.script, file = paste0(chr.wildlife.heregr.dir, "/", gsub("update", tmp.sub, chr.script.file.name)),
      sep = "\n")
