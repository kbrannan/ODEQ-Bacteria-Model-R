...
## for model
chr.input.dir <- chr.cowcalf.dir
chr.input.file <- chr.input

## watershed info
df.wtsd <- read.delim(
  paste0(gsub("/sub.*$","",chr.cowcalf.dir), "/sourcecontrol/wtsd-info.txt"), 
  sep=":", comment.char="*", stringsAsFactors=FALSE, header=FALSE)
names(df.wtsd) <- c("parameter","value")
unique(df.wtsd$parameter)
wtsd.rows <- grep("watershed", df.wtsd$parameter)
wtsd.row <- grep(gsub("[^0-9]","",chr.input.file), df.wtsd$value[wtsd.rows])
if(wtsd.row < length(wtsd.rows)) {
  df.sub.wtsd <- df.wtsd[(wtsd.rows[wtsd.row]):(wtsd.rows[wtsd.row + 1] - 1), ]  
} else {
  df.sub.wtsd <- df.wtsd[(wtsd.rows[wtsd.row]):length(df.wtsd[, 1]), ]
}

## run model
source(paste0(chr.cowcalf.dir,"/Cow_Calf_Sub_Model.R"))
df.output <- df.output <- cow.calf(chr.input.dir=chr.cowcalf.dir,
                                   chr.input.file=chr.input.file,
                                   lu.pasture.area = as.numeric(df.sub.wtsd$value[df.sub.wtsd$parameter == "Pasture area in acres"]),
                                   lu.forest.area = as.numeric(df.sub.wtsd$value[df.sub.wtsd$parameter == "Forest  area in acres"]))
## packages
