...
chr.script.file.name <- "Wildlife_Geese_Sub_Model_calculation_check-subwtsd-update.R"
chr.script <- scan(file = paste0(chr.wildlife.geese.dir, "/",
                                 chr.script.file.name),
                   what = "character", sep = "\n")

chr.files <- list.files(path = chr.wildlife.geese.dir, pattern = "^wildlifeGeese([0-9]){2}\\.txt$",
                        full.names = TRUE)
...
for(ii in 1:length(chr.files)) {
  ...
  chr.input <- paste0("wildlifeGeese", tmp.sub ,".txt")
...
##  cat(tmp.script, file = paste0(chr.wildlife.geese.dir, "/", gsub("update", tmp.sub, chr.script.file.name)),
##      sep = "\n")
