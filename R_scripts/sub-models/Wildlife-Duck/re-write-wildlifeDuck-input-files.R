...
chr.dir.wildlife.duck <- paste0(chr.rscripts.dir, "/sub-models/Wildlife-Duck")

chr.areas.file.name <- "Goose-Duck-habitat-by-PLS-sub-wtsd.csv"

chr.files <- list.files(path = chr.dir.wildlife.duck, pattern = "^wildlifeDuck([0-9]){2}\\.txt$",
                        full.names = TRUE)
...
df.areas.wq <- read.csv(file = paste0(chr.dir.wildlife.duck, "/", chr.areas.file.name))[, c(2, 3, 4)]
