...
chr.script.file.name <- "Wildlife_Coyote_Sub_Model_calculation_check-subwtsd-update.R"
chr.script <- scan(file = paste0(chr.wildlife.coyote.dir, "/",
                                 chr.script.file.name),
                   what = "character", sep = "\n")

chr.files <- list.files(path = chr.wildlife.coyote.dir, pattern = "^wildlifeCoyote([0-9]){2}\\.txt$",
                        full.names = TRUE)
...
for(ii in 1:length(chr.files)) {
  ...
  chr.input <- paste0("wildlifeCoyote", tmp.sub ,".txt")
  ...
  cat(tmp.script, file = paste0(chr.wildlife.coyote.dir, "/", gsub("update", tmp.sub, chr.script.file.name)),
      sep = "\n")
