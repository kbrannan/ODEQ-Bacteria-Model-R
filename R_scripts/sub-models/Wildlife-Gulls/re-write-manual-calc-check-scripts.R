...
chr.script.file.name <- "Wildlife_Gulls_Sub_Model_calculation_check-subwtsd-update.R"
chr.script <- scan(file = paste0(chr.wildlife.gulls.dir, "/",
                                 chr.script.file.name),
                   what = "character", sep = "\n")

chr.files <- list.files(path = chr.wildlife.gulls.dir, pattern = "^wildlifeGulls([0-9]){2}\\.txt$",
                        full.names = TRUE)
...
for(ii in 1:length(chr.files)) {
  ...
  chr.input <- paste0("wildlifeGulls", tmp.sub ,".txt")
  ...
  cat(tmp.script, file = paste0(chr.wildlife.gulls.dir, "/", gsub("update", tmp.sub, chr.script.file.name)),
      sep = "\n")
