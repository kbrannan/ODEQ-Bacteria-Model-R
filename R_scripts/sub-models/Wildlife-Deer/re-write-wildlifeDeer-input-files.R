...
chr.dir.wildlife.deer <- paste0(chr.rscripts.dir, "/sub-models/Wildlife-Deer")

chr.areas.file.name <- "sub-wtsd-areas.RData"

chr.files <- list.files(path = chr.dir.wildlife.deer, pattern = "^wildlifeDeer([0-9]){2}\\.txt$",
                        full.names = TRUE)
df.input <- read.delim(grep("01\\.txt$", chr.files, value = TRUE), sep=":",
                       stringsAsFactors=FALSE, header=FALSE)
names(df.input) <- c("parameter","value(s)")

load(file = paste0(chr.dir.wildlife.deer, "/", chr.areas.file.name))
