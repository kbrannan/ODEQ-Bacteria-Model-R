...
chr.script.file.name <- "Wildlife_Deer_Sub_Model_calculation_check-subwtsd-update.R"
chr.script <- scan(file = paste0(chr.wildlife.deer.dir, "/",
                                 chr.script.file.name),
                   what = "character", sep = "\n")

chr.files <- list.files(path = chr.wildlife.deer.dir, pattern = "^wildlifeDeer([0-9]){2}\\.txt$",
                        full.names = TRUE)
...
for(ii in 1:length(chr.files)) {
  ...
  chr.input <- paste0("wildlifeDeer", tmp.sub ,".txt")
  ...
  cat(tmp.script, file = paste0(chr.wildlife.deer.dir, "/", gsub("update", tmp.sub, chr.script.file.name)),
      sep = "\n")
