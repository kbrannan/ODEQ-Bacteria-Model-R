...
df.output <- cow.calf(chr.wrkdir=getwd(),chr.input.file="cowcalf18.txt")
## land use information
chk.lu.pasture.area <- 90.1 # in acres
chk.lu.forest.area  <- 2121.5 # in acres
chk.lu.pasture.w    <- 15.01685 # as percent
chk.lu.forest.w     <- 10.013 # as percent
...
chr.chk <- c(paste0("Standard Error for In-Stream     = ", 
             ...
             paste0("Standard Error for Forest Accum  = ", 
                    sum((chk.Accum.forest - df.output$Accum.Forest)^2)/12))
cat(chr.chk, file = "cow-calf-std-error-cowcalf18.txt", sep="\n")
