...
df.output <- cow.calf(chr.wrkdir=getwd(),chr.input.file="cowcalf02.txt")
## land use information
chk.lu.pasture.area <- 324.9 # in acres
chk.lu.forest.area  <- 6308.4 # in acres
chk.lu.pasture.w    <- 19.233492 # as percent
chk.lu.forest.w     <- 0.0043426 # as percent
...
chr.chk <- c(paste0("Standard Error for In-Stream     = ", 
             ...
             paste0("Standard Error for Forest Accum  = ", 
                    sum((chk.Accum.forest - df.output$Accum.Forest)^2)/12))
cat(chr.chk, file = "cow-calf-std-error-cowcalf02.txt", sep="\n")
