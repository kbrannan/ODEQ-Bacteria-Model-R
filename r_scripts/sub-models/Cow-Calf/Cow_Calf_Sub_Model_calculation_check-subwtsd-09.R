...
df.output <- cow.calf(chr.wrkdir=getwd(),chr.input.file="cowcalf09.txt")
## land use information
chk.lu.pasture.area <- 53 # in acres
chk.lu.forest.area  <- 2973.5 # in acres
chk.lu.pasture.w    <- 7 # as percent
chk.lu.forest.w     <- 1.6094129 # as percent
...
chr.chk <- c(paste0("Standard Error for In-Stream     = ", 
             ...
             paste0("Standard Error for Forest Accum  = ", 
                    sum((chk.Accum.forest - df.output$Accum.Forest)^2)/12))
cat(chr.chk, file = "cow-calf-std-error-cowcalf09.txt", sep="\n")
