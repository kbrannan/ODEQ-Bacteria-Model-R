...
df.output <- cow.calf(chr.wrkdir=getwd(),chr.input.file="cowcalf17.txt")
## land use information
chk.lu.pasture.area <- 197.5 # in acres
chk.lu.forest.area  <- 828 # in acres
chk.lu.pasture.w    <- 49 # as percent
chk.lu.forest.w     <- 0.21832975 # as percent
...
chr.chk <- c(paste0("Standard Error for In-Stream     = ", 
             ...
             paste0("Standard Error for Forest Accum  = ", 
                    sum((chk.Accum.forest - df.output$Accum.Forest)^2)/12))
cat(chr.chk, file = "cow-calf-std-error-cowcalf17.txt", sep="\n")
