...
df.output <- cow.calf(chr.wrkdir=getwd(),chr.input.file="cowcalf04.txt")
## land use information
chk.lu.pasture.area <- 195.2 # in acres
chk.lu.forest.area  <- 4868.1 # in acres
chk.lu.pasture.w    <- 32.074909 # as percent
chk.lu.forest.w     <- 0.00072642 # as percent
...
chr.chk <- c(paste0("Standard Error for In-Stream     = ", 
             ...
             paste0("Standard Error for Forest Accum  = ", 
                    sum((chk.Accum.forest - df.output$Accum.Forest)^2)/12))
cat(chr.chk, file = "cow-calf-std-error-cowcalf04.txt", sep="\n")
