...
df.output <- cow.calf(chr.wrkdir=getwd(),chr.input.file="cowcalf10.txt")
## land use information
chk.lu.pasture.area <- 11.5 # in acres
chk.lu.forest.area  <- 258.2 # in acres
chk.lu.pasture.w    <- 12.425484 # as percent
chk.lu.forest.w     <- 0.63668808 # as percent
...
chr.chk <- c(paste0("Standard Error for In-Stream     = ", 
             ...
             paste0("Standard Error for Forest Accum  = ", 
                    sum((chk.Accum.forest - df.output$Accum.Forest)^2)/12))
cat(chr.chk, file = "cow-calf-std-error-cowcalf10.txt", sep="\n")
