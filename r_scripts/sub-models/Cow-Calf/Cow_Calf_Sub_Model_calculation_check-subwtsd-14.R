...
df.output <- cow.calf(chr.wrkdir=getwd(),chr.input.file="cowcalf14.txt")
## land use information
chk.lu.pasture.area <- 218.1 # in acres
chk.lu.forest.area  <- 2245.3 # in acres
chk.lu.pasture.w    <- 5 # as percent
chk.lu.forest.w     <- 0.1478134 # as percent
...
chr.chk <- c(paste0("Standard Error for In-Stream     = ", 
             ...
             paste0("Standard Error for Forest Accum  = ", 
                    sum((chk.Accum.forest - df.output$Accum.Forest)^2)/12))
cat(chr.chk, file = "cow-calf-std-error-cowcalf14.txt", sep="\n")
