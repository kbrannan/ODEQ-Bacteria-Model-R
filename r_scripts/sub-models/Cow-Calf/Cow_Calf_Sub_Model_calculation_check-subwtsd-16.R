...
df.output <- cow.calf(chr.wrkdir=getwd(),chr.input.file="cowcalf16.txt")
## land use information
chk.lu.pasture.area <- 150.8 # in acres
chk.lu.forest.area  <- 2767.1 # in acres
chk.lu.pasture.w    <- 32 # as percent
chk.lu.forest.w     <- 0.51583584 # as percent
...
chr.chk <- c(paste0("Standard Error for In-Stream     = ", 
             ...
             paste0("Standard Error for Forest Accum  = ", 
                    sum((chk.Accum.forest - df.output$Accum.Forest)^2)/12))
cat(chr.chk, file = "cow-calf-std-error-cowcalf16.txt", sep="\n")
