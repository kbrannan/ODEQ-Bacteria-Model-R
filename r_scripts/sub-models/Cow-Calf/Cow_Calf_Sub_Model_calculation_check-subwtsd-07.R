...
df.output <- cow.calf(chr.wrkdir=getwd(),chr.input.file="cowcalf07.txt")
## land use information
chk.lu.pasture.area <- 185.5 # in acres
chk.lu.forest.area  <- 627.2 # in acres
chk.lu.pasture.w    <- 71 # as percent
chk.lu.forest.w     <- 0.03264901 # as percent
...
chr.chk <- c(paste0("Standard Error for In-Stream     = ", 
             ...
             paste0("Standard Error for Forest Accum  = ", 
                    sum((chk.Accum.forest - df.output$Accum.Forest)^2)/12))
cat(chr.chk, file = "cow-calf-std-error-cowcalf07.txt", sep="\n")
