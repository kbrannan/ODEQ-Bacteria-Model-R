...
df.output <- cow.calf(chr.wrkdir=getwd(),chr.input.file="cowcalf13.txt")
## land use information
chk.lu.pasture.area <- 475.9 # in acres
chk.lu.forest.area  <- 4882.5 # in acres
chk.lu.pasture.w    <- 25 # as percent
chk.lu.forest.w     <- 0.1890233 # as percent
...
chr.chk <- c(paste0("Standard Error for In-Stream     = ", 
             ...
             paste0("Standard Error for Forest Accum  = ", 
                    sum((chk.Accum.forest - df.output$Accum.Forest)^2)/12))
cat(chr.chk, file = "cow-calf-std-error-cowcalf13.txt", sep="\n")
