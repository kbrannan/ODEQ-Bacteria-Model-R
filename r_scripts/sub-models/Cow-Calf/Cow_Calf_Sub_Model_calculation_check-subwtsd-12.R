...
df.output <- cow.calf(chr.wrkdir=getwd(),chr.input.file="cowcalf12.txt")
## land use information
chk.lu.pasture.area <- 8.7 # in acres
chk.lu.forest.area  <- 1117.9 # in acres
chk.lu.pasture.w    <- 52.715915 # as percent
chk.lu.forest.w     <- 1.012111 # as percent
...
chr.chk <- c(paste0("Standard Error for In-Stream     = ", 
             ...
             paste0("Standard Error for Forest Accum  = ", 
                    sum((chk.Accum.forest - df.output$Accum.Forest)^2)/12))
cat(chr.chk, file = "cow-calf-std-error-cowcalf12.txt", sep="\n")
