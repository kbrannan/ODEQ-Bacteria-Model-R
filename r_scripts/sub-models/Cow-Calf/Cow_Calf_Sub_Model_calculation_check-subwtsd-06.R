...
df.output <- cow.calf(chr.wrkdir=getwd(),chr.input.file="cowcalf06.txt")
## land use information
chk.lu.pasture.area <- 146.2 # in acres
chk.lu.forest.area  <- 5365.3 # in acres
chk.lu.pasture.w    <- 31 # as percent
chk.lu.forest.w     <- 0.079494258 # as percent
...
chr.chk <- c(paste0("Standard Error for In-Stream     = ", 
             ...
             paste0("Standard Error for Forest Accum  = ", 
                    sum((chk.Accum.forest - df.output$Accum.Forest)^2)/12))
cat(chr.chk, file = "cow-calf-std-error-cowcalf06.txt", sep="\n")
